      ...
      <div className="relative grid grid-cols-1 gap-7 xl:grid-cols-[1fr_0.9fr] xl:items-end">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-aurora-blue/30 bg-aurora-blue/10 px-3 py-1 text-[11px] font-semibold tracking-wide text-cyan-300">Your mobility companion</span>
            <span className="font-mono text-[11px] text-slate-400">MOCK DATA PROTOTYPE</span>
          </div>
          <p className="mt-7 text-sm font-medium text-aurora-magenta">Welcome back, {firstName}</p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button type="button" onClick={onStartJourney} className="rounded-xl bg-aurora-blue px-4 py-2.5 text-sm font-bold text-white transition-all duration-200 hover:brightness-110 hover:shadow-lift">Ask the journey guide</button>
            <button type="button" onClick={onBrowseServices} className="rounded-xl border border-slate-700 bg-slate-900 px-4 py-2.5 text-sm font-semibold text-slate-300 transition-colors duration-200 hover:border-aurora-blue/40 hover:bg-slate-800">Browse all services</button>
          </div>
        </div>
