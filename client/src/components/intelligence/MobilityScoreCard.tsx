...
export function MobilityScoreCard({ snapshot }: MobilityScoreCardProps) {
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-sm">
      <h2 className="text-xl font-semibold text-slate-50">Mobility Health Score</h2>
      <div className="mt-6 flex items-center gap-6">
        <div className="relative h-28 w-28 shrink-0">
          <svg viewBox="0 0 100 100" className="h-full w-full -rotate-90">
            <circle cx="50" cy="50" r="42" fill="none" stroke="rgba(248,250,252,0.1)" strokeWidth="8" />
            <motion.circle
              cx="50"
              cy="50"
              r="42"
        ...
        <p className="max-w-md text-sm leading-6 text-slate-300">{snapshot ? snapshot.score.reasons.join(' · ') : 'Calculating from your linked vehicle and case records.'}</p>
      </div>
      {/* Plain conditional rendering, not AnimatePresence mode="wait" — same
        ...
        {snapshot ? (
          ...
          >
            ...
            {!snapshot.complianceAlerts.length ? <p className="text-sm text-emerald-400">No compliance actions are currently due.</p> : null}
          </motion.div>
        ) : null}
    </section>
  );
